...
struct YOLODecoder{
    ...
    /** Kenny Notes:
    * If you are receiving a confidence over 100, you mgiht have to convert the raw logits to probability
    * Check to see what your values of objectnessScore and maxClassScore are.
    * you might need something like this:
    * let objectnessScore = sigmoid(pointer[base + 4])
    * blah blah keep the rest the same
    *  for c in 0 ..< classNum {
    *   let score = sigmoid(pointer[base + 5 + c])
    ...
    *  }
    * 
    * Pay attention to how you are using/accessing pointers.
    * Your original implemnetation might not have been grabbing new data rather it was grabbing
    * stale data. You had a lot of memory overhead as well.
    * My next tip is this, optionals are great but know when to use them.
    * It's important to know when to be declarative vs not.
    **/
